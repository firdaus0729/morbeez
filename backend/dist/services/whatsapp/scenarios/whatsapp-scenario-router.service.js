...
function cropFromSelection(text) {
    ...
    if (/\bginger|inchi|ഇഞ്ചി|अदरक\b/i.test(t))
        return 'ginger';
    if (/\bbanana|vazha|വാഴ|केला\b/i.test(t))
        return 'banana';
    if (/\bcardamom|elakka|ഏലക്ക|इलायची\b/i.test(t))
        return 'cardamom';
    if (/\bpepper|kurumulaku|കുരുമുളക്|काली मिर्च\b/i.test(t))
        return 'pepper';
    if (/\bother|others|മറ്റ്|अन्य\b/i.test(t))
        return 'other';
    return null;
}
