...
export async function tryAgronomyReply(params) {
    ...
    if (!whatsappConversationalService.isEnabled()) {
        if (pair) {
            await params.sendText(params.phone, compatibilityLookupService.formatFarmerReply({ found: false, productA: pair.productA, productB: pair.productB }, params.language, pair));
            return true;
        }
        return false;
    }
