...
export const agronomistWorkflowService = {
    ...
    async listAgronomistSubmissions(status, limit = 50) {
        if (!status || status === 'all') {
            return recommendationRecordsService.listByStatus(['draft', 'pending_approval', 'approved', 'communicated', 'rejected'], limit);
        }
        return recommendationRecordsService.listByStatus(status, limit);
    },
};
